...
# EC2_PUBLIC_IP   = "YOUR_EC2_PUBLIC_IP"        # ← change this later when EC2 is ready
...
def run_edge():
    print(f"[EDGE] Loading YOLOv8n...")
    model = YOLO("yolov8n.pt")

    print(f"[EDGE] Opening video: {VIDEO_PATH}")
    cap = cv2.VideoCapture(VIDEO_PATH)
    ...
    while True:
        ...
        results   = model(frame, verbose=False)
        count     = sum(1 for box in results[0].boxes if int(box.cls[0]) in VEHICLE_CLASSES)
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        density   = classify_density(count)

        # write directly to CSV (no HTTP needed — same machine as Fog)
        with open(LOG_FILE, "a", newline="") as f:
            csv.writer(f).writerow([timestamp, frame_num, count, density])

        print(f"[EDGE] Frame {frame_num:>5} | Vehicles: {count:>3} | Density: {density}")

    cap.release()
    print("[EDGE] Done. Results saved to", LOG_FILE)
